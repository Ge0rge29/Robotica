...
unsigned long timpTastare[3] = {5000, 4000, 3000};
unsigned long timpJoc = 33000, momentSelectareCuvant = 0, timpDebounce = 300, timpIncepere = 3000, eroareIncepere = 50;
...
int verificaCuvant(const char *cuvantDeVerificat) {
    ...
    for (int i = 0; i < lungime; i++) {
        if (cuvantDeVerificat[i] != cuvantTinta[i]) return -1;
    }
    return lungime == int(strlen(cuvantTinta)) ? 1 : 0;
}
...
void incepere() {
    if (millis() - momentUltimaClipire > timpIncepere / 6) {
        ...
        if (!(clipiri % 2)) Serial.println(char(3 - clipiri / 2 + 48));
        clipiri++;
    }
    if (clipiri == 6) {
        index = 0;
